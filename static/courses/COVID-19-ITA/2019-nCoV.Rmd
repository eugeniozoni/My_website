...
```{r , warning=FALSE, message=FALSE}
df.expanded_ITA <- mydata_ITA[rep(row.names(mydata_ITA), mydata_ITA$new_positive_up_to_date), 2:16]
```
...
```{r , warning=FALSE, message=FALSE}
df.expanded_total_ITA <- mydata_ITA[rep(row.names(mydata_ITA), mydata_ITA$positive_total_up_to_date), 2:16]
```
...
plot(pred_ITA_new_before_shut) + labs(title="Future Incidence (R = 2.04) - based on growth of new positive cases in the first 15 days of diffusion")

set.seed(1)
pred_ITA_new_after_shut <- project(i_ITA[16:46], R = median(growth_R0_after), si = si_ITA_n, n_days = 10, n_sim = 1000)
pred_ITA_new_after_shut
plot(pred_ITA_new_after_shut) + labs(title="Future Incidence (R = 1.04) - based on new positive cases growth after shutdown")

set.seed(1)
pred_ITA_new_after_peak <- project(i_ITA[27:46], R = median(growth_R0_after_peak), si = si_ITA_n, n_days = 10, n_sim = 1000)
pred_ITA_new_after_peak
plot(pred_ITA_new_after_peak) + labs(title="Future Incidence (R = 0.89) - based on new positive cases growth after peak")
```
